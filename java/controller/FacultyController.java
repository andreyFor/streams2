...
public class FacultyController {
    ...
    public ResponseEntity<java.util.Collection<Faculty>> getAll() {
        java.util.Collection<Faculty> faculties = (java.util.Collection<Faculty>) facultyService.getAll();
        if (faculties == null) {
            return ResponseEntity.notFound().build();
        }
    ...
    public ResponseEntity<Collection> sortFacultiesByNameOrColor(@RequestParam(required = false) String name,
                                                                          @RequestParam(required = false) String color) {
        if (name != null && !name.isBlank()) {
            return ResponseEntity.ok((Collection) facultyService.findFacultiesByName(name));
        }
        if (color != null && !color.isBlank()) {
            return ResponseEntity.ok((Collection) facultyService.findFacultiesByColor(color));
        }
